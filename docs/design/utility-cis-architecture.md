...
**Custom fields:** Tenant-configurable custom fields on extendable entities (Customer, Account, Premise, ServiceAgreement, Meter) are stored in a per-entity `custom_fields` JSONB column and governed by a `custom_field_schema` row per (utility, entity_type) holding a `FieldDefinition[]` array. The shared `buildZodFromFields` helper dynamically builds a Zod validator from the tenant schema on every write, and the API's `validateCustomFields` service wraps it with merge semantics for partial updates, deprecated-field preservation, and strict rejection of unknown keys. Schema is cached in memory per tenant with a 60-second TTL, invalidated on mutation. See spec 20 for the full design and phase roadmap. Phase 1 ships the backend foundation and Customer as a pilot entity; Phases 2–4 extend to the other four entities, searchable expression indexes, and field-level RBAC.
