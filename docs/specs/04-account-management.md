...
- **Enforced at the database layer:** `deposit_amount >= 0` (CHECK constraint `account_deposit_amount_nonneg`). This cannot be bypassed even via direct SQL. See [Database Invariants](./00-data-model-overview.md#database-invariants-check-constraints) in the data model overview.
