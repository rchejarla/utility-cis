...
### Slice 2 — Async + notifications + cancel/retry
BullMQ `import-jobs` queue + worker. Sync/async threshold logic in the POST route. Wizard polls `/imports/:id` during PROCESSING and renders progress. Zombie-batch detection (startup sweep). `POST /imports/:id/cancel` (soft) and `/imports/:id/retry` (re-process error rows). `InAppNotification` row emitted on terminal transitions. Email via existing notification template system (new `import.complete` event type). User preference `notify_on_import_complete` (defaults true). Drop the 250-row implicit cap once the worker is in place; raise hard cap to 100k rows.
