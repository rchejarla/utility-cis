...
export default function AccountTypesPage() {
  ...
  const { canView, canCreate, canEdit } = usePermission("accounts");
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await apiClient.get<{ data: TypeDef[] }>(
        "/api/v1/account-types?includeInactive=true",
      );
      setRows(res.data ?? []);
    } catch (err) {
      console.error(err);
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await apiClient.post("/api/v1/account-types", {
      ...
      toast("Account type created", "success");
      setShowNew(false);
      setNewForm(EMPTY);
      fetchData();
    } catch (err: unknown) {
  ...
  const handleUpdate = async (id: string) => {
    ...
    setSaving(true);
    try {
      await apiClient.patch(`/api/v1/account-types/${id}`, {
  ...
  return (
    <div>
      <PageHeader
        title="Account Types"
        subtitle={`Billing-class taxonomy for accounts — Residential, Commercial, Government, etc. Standards are built in and read-only. (${standardCount} standard · ${customCount} custom)`}
        action={canCreate ? { label: "+ Add Account Type", onClick: () => setShowNew(true) } : undefined}
      />

      <PageDescription storageKey="account-types">
        An <b>account type</b> classifies the billing relationship — Residential,
        Commercial, Industrial, Municipal, Government, etc. It drives rate-
        schedule selection, deposit policies, and tax treatment. Standard codes
        are seeded for every tenant and cannot be edited; add your own codes
        alongside them (e.g., AGRICULTURAL, NONPROFIT_TAX_EXEMPT).
      </PageDescription>
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            ...
            {sorted.length === 0 && (
              <tr><td colSpan={7} style={{ padding: "24px 18px", color: "var(--text-muted)", textAlign: "center" }}>No account types defined.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function FormRow({ form, setForm, onSubmit, onCancel, busy, submitLabel }: { form: Form; setForm: (f: Form) => void; onSubmit: () => void; onCancel: () => void; busy: boolean; submitLabel: string }) {
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: 16, marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
      <div><div style={lblStyle}>Code</div><input style={{ ...inputStyle, width: 120, fontFamily: "monospace" }} value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="GOVERNMENT" /></div>
      <div><div style={lblStyle}>Label</div><input style={{ ...inputStyle, width: 200 }} value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="Government" /></div>
