...
export default function NotificationTemplatesPage() {
  ...
  if (loading) {
    return (
      <SettingsSection title="Notification Templates" description="Loading...">
  ...
  return (
    <SettingsSection
      ...
      description="Manage message templates for email and SMS notifications. Use {{variable}} syntax to insert dynamic values."
    >
      {/* Action bar */}
      {canCreate && (
...
function TemplateForm({ initial, onSave, onCancel }: TemplateFormProps) {
  ...
  // Live preview using declared sample values
  ...
  function renderPreview(text: string): string {
    return text.replace(/\{\{([^}]+)\}\}/g, (_, key: string) => {
      ...
      return sampleVars[trimmed] ?? `{{${trimmed}}}`;
    });
  }

  const previewSubject = emailSubject ? renderPreview(emailSubject) : "";
  const previewBody = activeTab === "email"
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        ...
        <div>
          ...
          <input
            style={{ ...settingInputStyle, width: "100%", fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}
            value={eventType}
            onChange={(e) => setEventType(e.target.value)}
            placeholder="delinquency.tier_1"
            disabled={!!initial}
          />
        </div>
      </div>
        ...
        {(["email", "sms"] as const).map((tab) => (
          <button
            ...
            style={{
              ...
              borderRadius: "var(--radius)",
              border: activeTab === tab ? "1px solid var(--accent-primary)" : "1px solid var(--border)",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <div>
          ...
          {activeTab === "sms" && (
            <>
              <Label>Body <span style={{ fontWeight: 400, color: "var(--text-muted)" }}>({smsBody.length}/160)</span></Label>
        ...
        <div>
          ...
          >
            {activeTab === "email" && previewSubject && (
              <div style={{ fontWeight: 600, marginBottom: 8, paddingBottom: 8, borderBottom: "1px solid var(--border-subtle)" }}>
                {previewSubject}
              </div>
            )}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={onCancel} style={{ padding: "8px 16px", fontSize: 13, background: "transparent", border: "1px solid var(--border)", borderRadius: "var(--radius)", color: "var(--text-secondary)", cursor: "pointer", fontFamily: "inherit" }}>
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={saving || !name || !eventType}
            style={{
              padding: "8px 20px",
              fontSize: 13,
              fontWeight: 600,
              ...
              border: "none",
              borderRadius: "var(--radius)",
              cursor: saving || !name || !eventType ? "not-allowed" : "pointer",
              fontFamily: "inherit",
            }}
          >
